...
def parse_args():
    ...
    parser.add_argument("-2", "--src_feat_name2", default="crepe")
...
def main(
    ...
    **kwargs,
    ...
    src_feat_model2 = FeatureExtractor(src_feat_name2, wav2vec_path, device)

    ref_feat_model = FeatureExtractor(ref_feat_name, wav2vec_path, device)

    print(f"[INFO] {src_feat_name} is loaded")
    print(f"[INFO] {src_feat_name2} is loaded")
        ...
        for pair_name, pair in tqdm(infos.items()):
            ...
            with torch.no_grad():
                ...
                src_feat = src_feat_model.get_feature([src_wav])[
                    0].unsqueeze(0)
                src_feat2 = src_feat_model2.get_feature([src_wav])[
                    0].unsqueeze(0)
                out_mel, attn = model(src_feat, src_feat2, tgt_mel)
