...
def parse_args():
    ...
    parser.add_argument("-2", "--src_feat2", type=str, default="crepe")
...
def model_fn(batch, model, criterion, device):
    ...
    srcs, src_masks, srcs2, src_masks2, tgts, tgt_masks, tgt_mels, overlap_lens = batch

    srcs = srcs.to(device)
    src_masks = src_masks.to(device)
    srcs2 = srcs2.to(device)
    src_masks2 = src_masks2.to(device)
    tgts = tgts.to(device)
...
def main(
    ...
    dataset = IntraSpeakerDataset3(
        data_dir, metadata_path, src_feat, src_feat2, ref_feat, n_samples, preload
    )
    input_dim, input_dim2, ref_dim, tgt_dim = dataset.get_feat_dim()
